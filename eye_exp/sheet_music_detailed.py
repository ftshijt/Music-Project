...
def Overall_split(pic_name):
    ...
    for i in range(len(index_continue)):
        if count_nonzero_index[index_continue[i][1]] - count_nonzero_index[index_continue[i][0]] < 150:
            index_continue_delete.append(i)
    for i in range(len(index_continue_delete)):
        index_continue.remove(index_continue[index_continue_delete[i]])
    ...
    cv.imwrite('img_split.png', img_arr) 
    
    plt.imsave('img_projection.png',img_projection)
...
def Voice_part_cut(rootFolderName, overrall_id, pic_id):
...
def Cut_into_voive_part(pic_name):
    ...
    for overrall_id in range(overall_num):
        ...
        for bar_id in range(bar_num):
            Voice_part_cut(pic_name, overrall_id, bar_id)
